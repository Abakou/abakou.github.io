...
export default function ContactForm(props) {
    ...
    if (s) {
        return <div
            ...
            <button className="bg-green-500 p-2 text-sm font-bold rounded-md px-4 hover:bg-green-700" onClick={() => setS(false)}><span className="pe-1">{t("Message sent")}</span> <CheckIcon className="inline" /></button>
        </div>
    }
    ...
    return <form action="" onSubmit={handleSubmit}>
        ...
        <button className='mt-5 font-mono dark:bg-yellow-600  bg-yellow-500  px-5 py-2.5 rounded-full hover:px-8 transition-all flex items-center uppercase '>
            {t("Post it")}
